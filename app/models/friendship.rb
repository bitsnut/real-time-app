class Friendship < ActiveRecord::Base
	...
	event :block do
		transitions from: [:pending, :requested, :accepted], to: :blocked
	end
	...
	def not_blocked
		if Friendship.exists?(user_id: user_id, friend_id: friend_id, state: 'blocked') ||
		   Friendship.exists?(user_id: friend_id, friend_id: user_id, state: 'blocked')
